...
#1) Initial distribution of behavioural types among the population:
...
#2) Probability of shyness/boldness in relation with age/ breeding expereince:
library(emmeans)
library(ordinal)
library(lsmeans)
...
#3) Probability of shyness/boldness in relation with clutch size: 
## is crafted from the same model: 
M.Behavior <- clmm(Behavior ~ Age * ClutchSize + Body.condition + hatching.success + ParasitizedNest + I(ClutchSize^2) + (1 | FemaleID), data = GD)
summary(M.Behavior)

# 3.1) post-hoc: Behavior and clutch size: 
...
# 3.2) Plot: Behaviour and clutch size: 
...
#4) Interaction effect of age and clutch size on behaviour:

M.Behavior <- clmm(Behavior ~ Age * ClutchSize + Body.condition + hatching.success + ParasitizedNest + I(ClutchSize^2) + (1 | FemaleID), data = GD)
summary(M.Behavior)
## 4.1) Emmeans: 
P1 <- data.frame(summary(emmeans(M.Behavior, pairwise~ Behavior | Age*ClutchSize,at = list(Age = 15, ClutchSize =19), mode = "prob")))
# 4.2) Plotting the interaction between age and clutch size in association with behaviour:
library(effects)
library(MASS)
...
#5) Associatino between behaviour and hatching success: 
...
# 5.1) To check the between-levels effect in behavior:
